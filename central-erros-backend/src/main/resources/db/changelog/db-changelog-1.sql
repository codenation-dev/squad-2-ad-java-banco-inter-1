...
-- changeset squad.2:1.2 dbms:mysql
            CREATE TABLE IF NOT EXISTS `centralerros`.`category` (
              `id` BIGINT(20) NOT NULL AUTO_INCREMENT,
              `name` VARCHAR(255) NOT NULL,
              `application_id` BIGINT(20) NULL,
              PRIMARY KEY (`id`),
              INDEX `fk_category_application_idx` (`application_id` ASC),
              CONSTRAINT `fk_category_application`
              ...
              CONSTRAINT `fk_event_application`
                ...
                CONSTRAINT `fk_event_category`
                    FOREIGN KEY (`category_id`)
                        REFERENCES `centralerros`.`category` (`id`)
                        ON DELETE NO ACTION
                        ON UPDATE NO ACTION)
    ENGINE = InnoDB;
